        ...
        for (uint i=0; i<parentNodes.length; i++) {
            ...
            if(currentNodeList.length == 17) {
                if(pathPtr == path.length) {
                    if(keccak256(RLP.toBytes(currentNodeList[16])) == keccak256(abi.encodePacked(value))) {
                        return true;
                    } else {
                      return false;
                    }
            ...
            pathPtr += _nibblesToTraverse(RLP.toData(currentNodeList[0]), path, pathPtr);
                ...
                if(pathPtr == path.length) {//leaf node
                    if(keccak256(RLP.toData(currentNodeList[1])) == keccak256(abi.encodePacked(value))) {
                        return true;
                    } else {
                        return false;
                    }
